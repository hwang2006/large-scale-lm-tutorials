...
if __name__ == "__main__":
    ...
    #batch_size, train_steps = 20, 4300
    #batch_size, train_steps = 24, 3600 #A100
    #batch_size, train_steps = 32, 2700 #A100
    #batch_size, train_steps = 48, 1800 #A100
    ...
    model = PipelineModule(
        ...
        #create_model_from_pretrained(model_name="gpt2-medium"),
        loss_fn=loss_fn,
        num_stages=world_size,
        #partition_method="type:GPT2Block"
        #partition_method="type:GPT2*"
    ...
    #datasets = load_dataset("squad").data["train"]["context"]  # 87599 
    ...
    tokenized_datasets = datasets.map(lambda x: tokenizer(x["context"], \
    ...
    data_loader = iter ( 
      DataLoader(
         #tokenized_datasets["train"]["input_ids"], shuffle=False, batch_size=batch_size, \
         sorted(tokenized_datasets["train"]["input_ids"], key=len, reverse=True), \
         shuffle=False, batch_size=batch_size, collate_fn=data_collator, num_workers=8,
      )
    )
    ...
    for i in range(num_training_steps):
        #loss = engine.train_batch(iter(data_loader))
