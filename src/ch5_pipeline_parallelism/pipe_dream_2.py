...
if __name__ == "__main__":
    ...
    def data_collator(batch):
       input_ids_batch_tensored = torch.tensor([x["input_ids"] for x in batch])
       return input_ids_batch_tensored 

    data_loader = iter ( 
      DataLoader(
       tokenized_datasets["train"], shuffle=False, batch_size=batch_size, \
       collate_fn=data_collator, num_workers=8,
      )
    )
