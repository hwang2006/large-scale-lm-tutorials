...
class MPU(object):
    ...
    def __init__(
        ...
        #print(f"Function reduce_fn: {self._reduce_fn}")
    ...
    def reduce(self, inputs: Tensor) -> Tensor:
        ...
        if self._enable_grad(inputs):
            print(f"enable_grad, before inputs: {inputs}") 
            outputs = self._reduce_fn.apply(inputs)
        else:
            print(f"not enable_grad, before inputs: {inputs}")
    ...
    def _initialize_functions(self):
        ...
        class Reduce(Function):
            @staticmethod
            def forward(ctx, inputs):
            #def forward(inputs):  # ERROR
                print("Reduce Function forward is called")
                return self._reduce(inputs)
